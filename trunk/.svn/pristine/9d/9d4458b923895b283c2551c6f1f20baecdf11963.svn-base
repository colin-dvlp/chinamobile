...
{      
	int brightness = LoadInteger("SecugenBrightness",200);
	//For OSP04 the range of brightness is 0x1 to 0x255.
	//For OPP03 the range of brightness is 0x0 to 65535

#ifdef OSP04
	OSP04_Initialize(brightness);
#else
	int gain = 100;
	int contrast = 20;
	int timeout = 3;
	int quality = LoadInteger("Quality",50);
	int detectinterval = LoadInteger("DetectInterval",3);
	OPP_Initialize(brightness, gain, contrast, timeout, quality, detectinterval);
#endif
	fd_sensor=7;
	return;
}
...
void ResizeImage(unsigned char *Inbuf, int SWidth, int SHeight, unsigned char *Outbuf)
{
	/* input image size if 258x336, output image size is 256*336 */
	int i, j;
	for(i=0;i<SHeight;i++)
		for(j=0;j<SWidth-2;j++)
			Outbuf[i*(SWidth-2)+j] = Inbuf[i*SWidth+j+1];
	return;
}

int CaptureSensor(char *Buffer, BOOL Sign, PSensorBufInfo SensorBufInfo)
{
/* for sensor osp04, the image size always is 258x336. for sensor opp03, the original image size if 300x400, the resampled image size is 260x300 */

	U32 framesize=ImageWidth*ImageHeight; //framesize = 260x300(opp03) or 258x336(osp04)
	static unsigned char ImgBuf[400*300];	//for original image
	U32 len;
	unsigned char temp[1024];
	int score;
//	DBPRINTF("READING IMAGE...\n");
#ifdef OSP04
	score = OPP_GetLiveImage (ImgBuf, 258*336);	//get image from secugen sensor
#else
	score = OPP_GetLiveImage (ImgBuf, 300*400);	//get image from secugen sensor
#endif

	if(score==OPP_OK)
	{
#ifndef OSP04
		Elim_Trape (ImgBuf, Buffer);  //re-sample picture
#else
/*
		if(BIOKEY_EXTRACT(fhdl, Buffer, temp,  EXTRACT_FOR_IDENTIFICATION)<=0)
		{
			printf("The image is not good!\n");
			return FALSE;
		} */

//		memcpy(Buffer,ImgBuf,framesize);
		/* Souce image is 258*336, Dest image is 256*336 */
		ResizeImage(ImgBuf,258,336,Buffer); 
#endif
		ExBeep(1);
		len = framesize;
//		len = 400*300;
